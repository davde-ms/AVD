...
Function ShutdownAzVMs {
  [CmdletBinding()]
  Param(
    [Parameter(Mandatory = $True)]
    [String]$ResourceGroupName,
    [Parameter(Mandatory = $True)]
    [String]$MaintenanceTimeWindowTag
  )
  $vms = Get-AzVM -ResourceGroupName $ResourceGroupName
  Foreach ($vm in $vms) {
    Get-AzVM -ResourceGroupName $ResourceGroupName -Name $vm | Select-Object -ExpandProperty OSProfile | Select-Object -ExpandProperty WindowsConfiguration | Select-Object -ExpandProperty TimeZone
    $vmTimeWindowTag = (Get-AzResource -ResourceId $vm.Id).$MaintenanceTimeWindowTag
    If ($vmTimeWindowTag) {
      $startTime = [datetime]::ParseExact($vmTimeWindowTag, "hh:mm tt", $null)
